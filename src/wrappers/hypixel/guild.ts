...
const main = `http://api.hypixel.net/guild?key=${KEYS[Math.floor(Math.random() * (KEYS.length - 1))]}`;
const endpoints = {
    player: main + "&player=",
    id: main + "&id=",
    name: main + "&name="
...
export default async function get(query, type, parseNames) {
            ...
            for (const [index, member] of members.entries()) {
                if (parseNames) {
                    ...
                    if (!sk1erName || !sk1erName.name) {
                        ...
                        data.guild.members[index].username = username[username.length - 1].name;
                    } else {
